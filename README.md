...
| [Chapter 2](ch2-spring-boot/README.md) | 스프링 부트 (학습 중) | Gradle 멀티 모듈 구성, REST 컨트롤러, GET 요청 매핑, 경로 변수, 쿼리 파라미터와 DTO 바인딩, POST 요청 본문과 JSON 필드 매핑, PUT 중첩 DTO와 JSON 응답, DELETE 요청 처리 |
...
└── ch2-spring-boot/
    ...
    └── response-api/         # HTTP 응답 학습용 기본 모듈
        ├── README.md
        ├── build.gradle
        └── src/main/         # 실행 클래스와 설정
```
