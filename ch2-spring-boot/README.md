...
현재 GET 요청 예제를 `get-api` 모듈로 구성했습니다. 이후 학습 프로젝트는 같은 위치에 새 모듈로 추가합니다.
...
테스트 보고서는 `get-api/build/reports/tests/test/index.html`에 생성됩니다. 여러 모듈을 동시에 실행하려면 모듈마다 다른 포트를 지정합니다.
...
예를 들어 다음 학습 프로젝트를 `post-api`로 추가할 때는 아래 순서로 구성합니다. `post-api`는 추가 방법을 설명하기 위한 이름이며 현재 등록된 모듈은 `get-api`입니다.

1. `ch2-spring-boot/post-api/`에 `build.gradle`, `src/main/java`, `src/main/resources`, `src/test/java`를 만듭니다.
2. 루트 `settings.gradle`에 `include 'post-api'`를 추가합니다.
...
5. `src/main/resources/application.properties`에 `spring.application.name=post-api`를 설정합니다. 다른 모듈과 동시에 실행한다면 `server.port=8081`처럼 별도 포트도 설정합니다.
6. `./gradlew :post-api:build`, `./gradlew :post-api:bootRun`으로 검증·실행하고 이 문서의 모듈 목차에 링크를 추가합니다.
...
3. Gradle 창에 `get-api` 하위 프로젝트가 나타나는지 확인합니다.
4. `get-api` 모듈의 `GetApplication.main()` 또는 `get-api → Tasks → application → bootRun`을 실행합니다.
