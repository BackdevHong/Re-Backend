...
├── delete-api/
│   ├── README.md                # DELETE API 학습 내용과 실행 방법
│   ├── build.gradle
...
테스트 보고서는 `get-api/build/reports/tests/test/index.html`에 생성됩니다. `get-api`는 기본 포트 8080, `post-api`는 8081, `put-api`는 8082, `delete-api`는 8083, `response-api`는 8084, `object-mapper-api`는 8085를 사용합니다. 실행 시 다른 포트로 변경할 수도 있습니다.

```bash
./gradlew :get-api:bootRun --args='--server.port=8086'
```
...
예를 들어 다음 학습 프로젝트를 `new-api`로 추가할 때는 아래 순서로 구성합니다. `new-api`는 추가 방법을 설명하기 위한 이름이며 현재 등록된 모듈은 `get-api`, `post-api`, `put-api`, `delete-api`, `response-api`, `object-mapper-api`입니다.
...
5. `src/main/resources/application.properties`에 `spring.application.name=new-api`를 설정합니다. 다른 모듈과 동시에 실행한다면 `server.port=8086`처럼 별도 포트도 설정합니다.
...
3. Gradle 창에 `get-api`, `post-api`, `put-api`, `delete-api`, `response-api`, `object-mapper-api` 하위 프로젝트가 나타나는지 확인합니다.
4. 실행할 모듈의 `GetApplication.main()`, `PostApplication.main()`, `PutApplication.main()`, `DeleteApplication.main()`, `ResponseApplication.main()`, `ObjectMapperApplication.main()`을 실행합니다. Gradle 창에서 해당 모듈의 `Tasks → application → bootRun`으로도 실행할 수 있습니다.
