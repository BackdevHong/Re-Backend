...
POST API 학습을 위한 기본 Spring Boot 모듈입니다. 현재는 실행 클래스와 설정만 있으며 컨트롤러, DTO, API 예제는 추가하지 않았습니다.
...
서버는 `http://localhost:8081`에서 시작합니다. 등록된 API가 없으므로 기본 경로 요청에는 `404`가 반환됩니다. 종료는 실행 터미널에서 `Ctrl+C`를 누릅니다.
